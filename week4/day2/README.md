...
## Summary
Explored how k-fold cross-validation produces a more reliable performance estimate than a single train/test split. Practiced running 5-fold cross-validation with cross_val_score on a Logistic Regression pipeline, comparing it to a single train/test split, and confirming the role of stratified folds on the Breast Cancer Wisconsin dataset.
...
- cross_val_score: interpreting the mean and standard deviation across folds
...
- Ran 5-fold cross-validation with cross_val_score, obtaining five F1 scores: 0.954, 0.986, 0.938, 0.971, and 0.957.
...
- Confirmed that cross_val_score uses stratified folds by default for classifiers when cv is specified as an integer, and explained that the difference in results came from enabling shuffling in the explicit StratifiedKFold.
